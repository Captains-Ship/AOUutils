...
class Rule(commands.Cog):
        ...
        try:
            if rule == -1:
                ...
                for i in range(0, rulength):
                    embed.add_field(name=f'Rule #{i + 1}', value=self.ruleshort[i], inline=False)
    ...
    async def enforce(self, ctx, user: discord.Member=None, rule: int=9999):
        ...
        try:
            ...
            await ctx.send(f'{user.mention} Please follow our rules. You have been warned for rule {rule}:', embed=embed)
            await user.send(f'{user.mention} Please follow our rules. You have been warned for rule {rule}:', embed=embed)
